...
export async function POST(request: Request) {
  try {
    ...
    if (!idToken || !nonce) {
      return NextResponse.json({ error: "idToken/nonce가 필요합니다." }, { status: 400 });
    }
    ...
    const { data: authData, error: authError } = await anon.auth.signInWithIdToken({
      provider: "apple",
      token: idToken,
      nonce,
    });
    ...
    if (authError || !authData?.session || !authData.user) {
      return NextResponse.json(
        { error: authError?.message ?? "Apple 로그인 세션 교환에 실패했습니다." },
        { status: 401 }
      );
    }
    ...
    const provider =
      typeof user.app_metadata?.provider === "string" ? user.app_metadata.provider : "apple";
