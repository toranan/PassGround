...
function buildDisplayName(
  ...
  const fromMeta = [metadata.nickname, metadata.name, metadata.full_name, metadata.user_name].find(
...
export async function POST(request: Request) {
  try {
    ...
    if (!idToken) {
      return NextResponse.json({ error: "카카오 ID 토큰이 없습니다." }, { status: 400 });
    }
    ...
    if (signInError || !signInData?.session || !signInData.user) {
      return NextResponse.json(
        {
          error: signInError?.message ?? "카카오 네이티브 로그인 교환에 실패했습니다.",
        },
        { status: 401 }
      );
    }
    ...
    const provider =
      typeof user.app_metadata?.provider === "string" ? user.app_metadata.provider : "kakao";
